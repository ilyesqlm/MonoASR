...
dataset_kabyle = load_dataset(kabyle_dataset_name, trust_remote_code=True)
dataset_arabic = load_dataset(arabic_dataset_name, trust_remote_code=True)
dataset_french = load_dataset(french_dataset_name, trust_remote_code=True)
dataset_wolof = load_dataset(wolof_dataset_name, trust_remote_code=True)
dataset_yoruba = load_dataset(yoruba_dataset_name, trust_remote_code=True)
...
dataset_kabyle = dataset_kabyle.map(convert_audio_format)
dataset_wolof = dataset_wolof.map(convert_audio_format)
dataset_yoruba = dataset_yoruba.map(convert_audio_format)

dataset_kabyle = dataset_kabyle.remove_columns(["Licence"])
dataset_arabic = dataset_arabic.remove_columns(["syllables", "syllable_classes"])
dataset_french = dataset_french.remove_columns(["english_transcript", "labels", "input_features"])
dataset_yoruba = dataset_yoruba.remove_columns(["speaker_id"])
...
dataset_kabyle = dataset_kabyle.map(remove_special_characters)
dataset_arabic = dataset_arabic.map(remove_special_characters)
dataset_french = dataset_french.map(remove_special_characters)
dataset_wolof = dataset_wolof.map(remove_special_characters)
dataset_yoruba = dataset_yoruba.map(remove_special_characters)

def clean_text(batch):
    batch["Text"] = [re.sub(r'[\xa0\u202f]', ' ', text) for text in batch["Text"]]
    return batch

dataset_kabyle = dataset_kabyle.map(clean_text, batched=True)


def remove_newlines(batch):
    batch["Text"] = batch["Text"].replace("\n", " ") 
    return batch
dataset_french = dataset_french.map(remove_newlines)


kab_train = dataset_kabyle["train"]
kab_test  = dataset_kabyle["test"]

arab_train = dataset_arabic["train"]
arab_test  = dataset_arabic["test"]

frc_train = dataset_french["train"]
frc_test  = dataset_french["test"]  

wol_train = dataset_wolof["train"]
wol_test  = dataset_wolof["test"]

yor_train = dataset_yoruba["train"]
yor_test  = dataset_yoruba["test"]
...
kab_train = kab_train.cast(common_features)
arab_train = arab_train.cast(common_features)
frc_train = frc_train.cast(common_features)
wol_train = wol_train.cast(common_features)
yor_train = yor_train.cast(common_features)

kab_test  = kab_test.cast(common_features)
arab_test = arab_test.cast(common_features)
frc_test  = frc_test.cast(common_features)
wol_test  = wol_test.cast(common_features)
yor_test  = yor_test.cast(common_features)

total_train = concatenate_datasets([kab_train, arab_train, frc_train, wol_train, yor_train])
total_test  = concatenate_datasets([kab_test,  arab_test,  frc_test,  wol_test,  yor_test ])
