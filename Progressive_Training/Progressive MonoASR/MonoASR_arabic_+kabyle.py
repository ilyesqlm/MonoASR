...
class UniWav(nn.Module):
  ...
  def forward(self, audio, labels=None, language="arabic"):
...
def train_epoch(model, train_loader, optimizer, lr_scheduler, step, gradient_accumulation_steps: int = 8, language="arabic"):
    loss_meter = AvgMeter()
...
def val_epoch(model, val_loader, processor, wer_metric, bleu_metric, rouge_metric, ignore_index=-100, language="arabic"):
...
def main():
    ...
    uniwav.load_state_dict(torch.load("../A1_kabyle_with_encoder/best.pt", map_location=CFG.device))
