...
class UniWav(nn.Module):
  ...
  def forward(self, audio, labels=None, language="yoruba"):
...
def load_yoruba_dataset(subset=CFG.yoruba_dataset_name):
    ...
    return final_dataset['train'], final_dataset['validation'], final_dataset['test']
...
def train_epoch(model, train_loader, optimizer, lr_scheduler, step, gradient_accumulation_steps: int = 8, language="yoruba"):
    loss_meter = AvgMeter()
...
def val_epoch(model, val_loader, processor, wer_metric, bleu_metric, rouge_metric, ignore_index=-100, language="yoruba"):
...
def test_epoch(model, test_loader, processor, wer_metric, bleu_metric, rouge_metric, ignore_index=-100, language="yoruba"):
...
def main():
    ...
    train_dataset_yoruba, val_dataset_yoruba, test_dataset_yoruba = load_yoruba_dataset()
    ...
    test_loader_yoruba = build_audio_loaders(test_dataset_yoruba, processor)
    ...
    uniwav.load_state_dict(torch.load("../GPAfrica_unispeech_prog_wolof/best_wol.pt", map_location=CFG.device))
    ...
    wolof_loader_cycle = cycle(train_loader_wolof)
    ...
    for epoch in range(CFG.epochs):
        ...
        optimizer.zero_grad()
        compt = 0
        for step, batch in enumerate(tqdm(train_loader_yoruba, desc=f"yoruba Epoch {epoch+1}")):
            audio = {k: v.to(CFG.device) for k, v in batch.items() if k != "input_ids"}
            labels = {k: v.to(CFG.device) for k, v in batch.items() if k == "input_ids"}

            outputs = uniwav(audio, labels, language="yoruba")
            ...
            if (step + 1) % CFG.gradient_accumulation_steps == 0:
                ...
                if compt == 3:
                    ...
                    for _ in range(8):
                        wolof_batch = next(wolof_loader_cycle)
                        audio_k = {k: v.to(CFG.device) for k, v in wolof_batch.items() if k != "input_ids"}
                        labels_k = {k: v.to(CFG.device) for k, v in wolof_batch.items() if k == "input_ids"}

                        outputs_k = uniwav(audio_k, labels_k, language="wolof")
                        loss_k = outputs_k["loss"] / CFG.gradient_accumulation_steps
                        loss_k.backward()
        ...
        avg_train_loss = running_loss / len(train_loader_yoruba)
        print(f"Epoch: {epoch+1} train loss: {avg_train_loss:.4f}")

        uniwav.eval()

        with torch.no_grad():
            print("Validation on yoruba...")
            val_loss_yoruba, wer_yoruba, bleu_yoruba, rouge1_yoruba, rouge2_yoruba, rougeL_yoruba = val_epoch(
                uniwav, val_loader_yoruba, processor, wer_metric, bleu_metric, rouge_metric,
                ignore_index=CFG.ignore_index, language="yoruba"
            )
            print(
                f"[yoruba Validation Epoch {epoch+1}] "
                f"Loss: {val_loss_yoruba.avg:.4f} | "
                f"WER: {wer_yoruba.avg:.4f} | "
                f"BLEU: {bleu_yoruba.avg:.4f} | "
                f"ROUGE-1: {rouge1_yoruba.avg:.4f} | "
                f"ROUGE-2: {rouge2_yoruba.avg:.4f} | "
                f"ROUGE-L: {rougeL_yoruba.avg:.4f}"
            )
        ...
        if wer_yoruba.avg < best_wer_yoruba:
            best_wer_yoruba = wer_yoruba.avg
            torch.save(uniwav.state_dict(), "best_yoruba.pt")
            print("Saved best yoruba model!")
        ...
        lr_scheduler.step(val_loss_yoruba.avg)
    ...
    print("\nLoading best yoruba model for final test...")
    uniwav.load_state_dict(torch.load("best_yoruba.pt"))
    uniwav.eval()
    with torch.no_grad():
        print("Testing on yoruba...")
        test_loss_yoruba, wer_yoruba, bleu_yoruba, rouge1_yoruba, rouge2_yoruba, rougeL_yoruba = val_epoch(
            uniwav, test_loader_yoruba, processor, wer_metric, bleu_metric, rouge_metric,
            ignore_index=CFG.ignore_index, language="yoruba"
        )
        print(f"yoruba Test -> Loss: {test_loss_yoruba.avg:.4f}, WER: {wer_yoruba.avg:.4f}, "
            f"BLEU: {bleu_yoruba.avg:.4f}, ROUGE-1: {rouge1_yoruba.avg:.4f}, "
            f"ROUGE-2: {rouge2_yoruba.avg:.4f}, ROUGE-L: {rougeL_yoruba.avg:.4f}")
        ################################################
