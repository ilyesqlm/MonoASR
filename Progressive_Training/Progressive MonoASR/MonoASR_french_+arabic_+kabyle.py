...
##Dataset arabic
def load_arabic_dataset(subset=CFG.arabic_dataset_name):
    dataset = load_dataset(subset, trust_remote_code=True, split={"train": "train[:80%]", "test": "test[:80%]"})
    dataset = dataset.cast_column("audio", Audio(sampling_rate=16_000))
    dataset = dataset.map(remove_special_characters)
    return dataset['train'], dataset['test']

##Dataset Kabyle
def remove_special_characters_kabyle(batch):
    batch["Text"] = re.sub(chars_to_remove_regex, '', batch["Text"]).lower()
    return batch

def clean_text_kabyle(batch):
    batch["Text"] = [re.sub(r'[\xa0\u202f]', ' ', text) for text in batch["Text"]]
    return batch

def load_kabyle_dataset(subset=CFG.kabyle_dataset_name):
    dataset = load_dataset(subset, trust_remote_code=True, split={"train": "train[:10%]", "test": "test[:10%]"})
    dataset = dataset.cast_column("audio", Audio(sampling_rate=16_000))
...
def main():
    ...
    train_dataset_arabic, val_dataset_arabic = load_arabic_dataset()
    ...
    train_dataset_kabyle, val_dataset_kabyle = load_kabyle_dataset()
    ...
    train_dataset_french, val_dataset_french = load_french_dataset()
    ...
    uniwav.load_state_dict(torch.load("../Architecture1_arabic_+kabyle/best_ar.pt", map_location=CFG.device))
    ...
    for epoch in range(CFG.epochs):
        ...
        mod = (epoch + 1) % 3
        if mod == 0:
            current_train_loader = train_loader_kabyle
            current_val_loader   = val_loader_kabyle
            train_language       = "kabyle"
        elif mod == 1:
            current_train_loader = train_loader_arabic
            current_val_loader   = val_loader_arabic
            train_language       = "arabic"
        else:
            current_train_loader = train_loader_french
            current_val_loader   = val_loader_french
            train_language       = "french"


        # Set the model in train mode
        uniwav.train()
        train_loss = train_epoch(uniwav, current_train_loader, optimizer, lr_scheduler, "epoch", gradient_accumulation_steps=CFG.gradient_accumulation_steps, language=train_language)
        print(f"Epoch: {epoch+1}, language: {train_language}, train loss: {train_loss}")
        ...
        with torch.no_grad():
            ...
            print(f" {epoch + 1}, language: {train_language},  {val_loss},  {wer},  {bleu},  {rouge1},  {rouge2},  {rougeL}")

        if mod == 0:
            if val_loss.avg < best_loss_kab:
                best_loss_kab = val_loss.avg
                #num_bad_epochs = 0
                torch.save(uniwav.state_dict(), "best_kab.pt")
                print("Saved best kabyle model!")
            
        elif mod == 1:
            if val_loss.avg < best_loss_ar:
                best_loss_ar = val_loss.avg
                #num_bad_epochs = 0
                torch.save(uniwav.state_dict(), "best_ar.pt")
                print("Saved best arabic model!")
        else:
            if val_loss.avg < best_loss_fr:
                best_loss_fr = val_loss.avg
                #num_bad_epochs = 0
                torch.save(uniwav.state_dict(), "best_fr.pt")
