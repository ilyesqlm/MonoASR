...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        text = self.datasets[index]["transcription"]
...
def clean_text(batch):
    batch["transcription"] = [re.sub(r'[\xa0\u202f]', ' ', text) for text in batch["transcription"]]
    return batch
...
def train_epoch(model, train_loader, optimizer, lr_scheduler, step, gradient_accumulation_steps: int = 4, language="arabic"):
    loss_meter = AvgMeter()
...
def main(train_language="arabic"):
    # Load processor
    processor = AutoProcessor.from_pretrained(CFG.processor_name)

    # Load dataset
    train_dataset, val_dataset = load_arabic_dataset()
    ...
    for epoch in range(CFG.epochs):
        ...
        uniwav.train()
        train_loss = train_epoch(uniwav, train_loader, optimizer, lr_scheduler, "epoch", gradient_accumulation_steps=CFG.gradient_accumulation_steps, language="arabic")
