...
def load_yoruba_dataset(subset=CFG.yoruba_dataset_name, seed=42):
...
def train_epoch(model, train_loader, optimizer, lr_scheduler, step, gradient_accumulation_steps = CFG.gradient_accumulation_steps, language="yoruba"):
    loss_meter = AvgMeter()
...
def main(train_language="yoruba"):
    ...
    # Load dataset
    train_dataset, val_dataset, test_dataset = load_yoruba_dataset()
    ...
    languages = ["kabyle", "arabic", "french", "wolof"]
    ...
    for epoch in range(CFG.epochs):
        ...
        uniwav.train()
        train_loss = train_epoch(uniwav, train_loader, optimizer, lr_scheduler, "epoch", gradient_accumulation_steps=CFG.gradient_accumulation_steps, language=train_language)
